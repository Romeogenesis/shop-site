...
def admin_dashboard():
    ...
    if request.method == 'POST':
        ...
        try:
            if action == 'list_users':
                users = admin.get_all_users() or []
                message = f"Загружено {len(users)} пользователей"
            elif action == 'list_products':
                products = admin.get_all_products() or []
                ...
                if name and password:
                    if admin.add_user(name, password, role):
                ...
                if not name or not price_str:
                    ...
                    try:
                        ...
                        elif admin.add_product(name, price):
    ...
    # Обновляем списки после изменений
    users = users or (admin.get_all_users() or [])
    products = products or (admin.get_all_products() or [])
...
{% for p in products %}<tr><td>{{ p.id }}</td><td>{{ p.product_name }}</td><td>{{ p.price }} ₽</td></tr>{% endfor %}
</table></div>
{% endif %}
